 ...
 ** @details Для зашифрования и расшифрования сообщения предназначены методы encrypt и decrypt. Текст передаётся на последовательное зашифрование и расшифрование. Доступна отмена выполнения программы.
 ** @warning Реализован только английский язык
 */
...
public:
     ...
     ** @warning Текст не должен быть пустой строкой и не должен содержать пробелы. В тексте могут содержаться знаки перпинания и цифры.
     ** @return result Расшифрованный текст
     ** @throw cypher_error Текст является пустой строкой
     */
